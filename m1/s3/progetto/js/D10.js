...
let string2 = "I love coding"
function deleteOne(string2, giusta) {
  if (giusta) {
...
console.log(deleteOne(string2, true))
...
*/
let string3 = "I have 4 dogs"
function onlyLetters(string3) {
  return string3.replace(/\d+/g,'')
}
...
*/
let string4 = "nomecognome@gmail.it"
function isThisAnEmail(string4) {
  let verifiedEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  return verifiedEmail.test(string4)
  }
console.log(isThisAnEmail(string4));
...
function whatDayIsIt(giorni) {
  ...
  let giornoIndex = currentDate.getDay()
...
function howManyDays(date) {
  let today = new Date()
  let timePassed = today.getTime() - date.getTime();
...
function isTodayMyBirthday(birthday) {
  ...
  return thisDay === birthdayDay && thisMonth === birthdayMonth
}
...
function onlyTheYears(movies) {
  return movies.map (obj => obj.Year)
}
...
function onlyInLastMillennium(movies) {
  return movies.filter(obj => parseInt(obj.Year)<2000)
}
...
function searchByTitle(movies, word) {
  return movies.filter(obj => obj.Title.includes(word))
}
...
function contenutoTd() {
  ...
  for (let index = 0; index < everyTd.length; index++) {
    console.log(everyTd[index].innerText)
  }
}
contenutoTd()
...
function addClassToTr() {
  ...
  everyTr.forEach(obj => obj.classList.add("test"))
}
addClassToTr()
